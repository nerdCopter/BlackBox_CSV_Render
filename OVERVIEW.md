...
1.  **Argument Parsing (`src/main.rs`):**
    ...
    * **BBL input expansion (`src/data_input/bbl_reader.rs`):** a `.bbl`/`.BBL` input (or its `.bfl`/`.BFL` synonym — the raw SD-card filename some flight controllers use for the same binary log format) is decoded via the `bbl_parser` crate and exported to a scratch CSV/`.headers.csv` pair per flight, reusing the existing CSV parser unchanged instead of duplicating its motor/eRPM channel-alignment logic. A multi-flight `.bbl`/`.bfl` expands to multiple entries (matching the `.01.csv`/`.02.csv` convention). Output defaults to the source file's own folder, not the scratch directory; scratch files are removed after that file's own processing.
...
* **Purpose:** Reports how a flight's stick input was distributed and how much rate headroom was used — raw data only, no tuning advice (`src/data_analysis/stick_distribution.rs`). Always computed, not gated by any plot-selection flag. No plot; `rc_command` over time is shown by `plot_rc_command_activity` when that plot is enabled.
* **Deflection zones:** Center (<15%), Mid (15–75%), High (≥75%), and Saturation (≥95%, a subset of High, not additional to it) — all as a % of true full-stick `|rc_command|` (500, the fixed clamp both Betaflight and EmuFlight apply before the rate curve), not this flight's own peak. Peak Stick (the flight's own observed max, as a % of that same 500) is reported alongside for context. Saturation Events counts separate excursions into that zone, not cumulative time.
* **Center reversal rate:** RC Command sign changes while continuously inside the Center zone, in Hz, relative to time spent in that zone (not total flight time).
* **Rate utilization:** P95 Setpoint vs. P95 Gyro Achieved per axis (95th percentile, not raw max, to avoid crash/tumble outliers). No per-instant tracking-error metric here — see Step Response Analysis for tracking quality.
* **Configured max rate / Rate Headroom:** `src/data_analysis/rate_curve.rs` computes the setpoint at full stick deflection from the header's `rates_type`/`rc_rates`/`rc_expo`/`rates`/`rate_limits` fields — all 5 Betaflight rate curve families, byte-identical to EmuFlight's `fc_rc.c` for the 4 it shares. `None` when `rc_rates`, `rc_expo`, or `rates` is missing or malformed, or when `rate_limits` is present but malformed. An absent `rate_limits` defaults to 1998 deg/s and an absent `rates_type` defaults to Betaflight. It is also `None` for an axis whose parsed values give a negative rate limit or a non-finite result. Rate Headroom is the unused % of Configured Max Rate (100% minus P95 Setpoint's % of it).
* **Report:** `## Stick Position & Rate Analysis`, omitted only when no axis has at least two usable RC Command samples.
