...
* **Deflection zones:** Each RC Command sample is expressed as a % of that axis's own peak `|rc_command|` for the flight — not a configured max-rate value. No rate-curve formula (Betaflight/Raceflight/KISS/Actual all differ, and diverge further between Betaflight and EmuFlight) is reconstructed from header metadata; reconstructing one incorrectly would be worse than not having the number. Center (<15%), Mid (15–75%), and High (>75%) zone time is reported as a % of total flight time, weighted by each sample's own held duration (the same sample-and-hold weighting `detect_rc_command_steps` uses for plateau timing) — not by sample count. Saturation (>95%) time is reported in seconds, as a subset of High.
