...
* **Configured max rate / Rate Headroom:** `src/data_analysis/rate_curve.rs` computes the setpoint at full stick deflection from the header's `rates_type`/`rc_rates`/`rc_expo`/`rates`/`rate_limits` fields — all 5 Betaflight rate curve families, byte-identical to EmuFlight's `fc_rc.c` for the 4 it shares. `None` when `rc_rates`, `rc_expo`, `rates`, or `rate_limits` is missing or malformed. Rate Headroom is the unused % of Configured Max Rate (100% minus P95 Setpoint's % of it).
* **Report:** `## Stick Position & Rate Analysis`, omitted only when no axis has RC Command samples with a positive peak.
