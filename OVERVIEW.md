...
* **Purpose:** Reports how a flight's stick input was distributed and how much rate headroom was used — raw data only, no tuning advice (`src/data_analysis/stick_distribution.rs`). Always computed, not gated by any plot-selection flag. No plot; `rc_command` is already visualized over time by `plot_rc_command_activity`.
