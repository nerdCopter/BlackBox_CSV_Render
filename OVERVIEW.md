...
* **Rate utilization:** P95 Setpoint vs. P95 Gyro Achieved per axis — the 95th percentile, not the raw maximum. A single crash/tumble sample can put raw max gyro rate an order of magnitude above every other sample in the flight (the same outlier problem `plot_setpoint_vs_gyro`'s Y-axis scaling guards against). No derived "tracking error" percentage is reported: comparing the two P95 values as independent aggregates misses genuine per-instant divergence (and can look fine even when it isn't), while a true per-instant computation is dominated by normal filter/motor lag already covered by the existing cross-correlation-based filtering-delay analysis in Gyro Analysis — reporting a second, cruder version of the same thing would be redundant at best and misleading at worst.
