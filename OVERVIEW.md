        ...
        * **P:D Ratio Recommendations (`src/main.rs`):**
            ...
            * **Other plots generated (`src/plot_functions/`):** Which plots are generated is controlled by the `PlotConfig` struct. The default (no flag) enables the core set; `--extended` enables all plots except Bode. Plots are gated per-field in `PlotConfig`:
                ...
                * **Extended plots (`--extended` adds these to the core set):**
                    ...
                    * `plot_stick_distribution`: Per-axis time-weighted histogram of RC Command (stick position) deflection, plus Setpoint-vs-Gyro rate utilization. See [Stick Position Distribution](#stick-position-distribution) below.
...
* **Histogram plot:** `*_Stick_Distribution_stacked.png` draws this same time-weighting as a 20-bin (5%-wide) step histogram per axis, with reference lines at the 15/75/95% zone boundaries.
...
- **`*_Stick_Distribution_stacked.png`** — Per-axis time-weighted histogram of RC Command (stick position) deflection, 0-100% of that axis's own peak for the flight, with reference lines at the Center/High/Saturation zone boundaries; see [Stick Position Distribution](#stick-position-distribution).
