...
- **Future Enhancement (Motor Torque Calculations):** Motor torque-based physics calculations are not currently implemented. Such calculations would require ESC voltage/current telemetry not available in standard Blackbox logs and could reintroduce circular logic issues. The current frame-class empirical approach provides accurate results without these parameters. Motor parameters may be added in future versions for advanced experimentation and validation.
