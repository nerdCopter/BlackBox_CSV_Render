...
2.  **File Processing (`src/main.rs:process_file`):**
    * For each input CSV:
        ...
        * **Time-Window Trim (optional `--start`/`--end`):** When either flag is given, rows outside the requested window (seconds relative to the log's first row) are dropped from the in-memory dataset before any analysis or plot runs, so every module — step response, gyro/motor/D-term analysis, desync detection — sees only the trimmed subset. Useful for zooming into a short event (e.g. a brief motor desync near the end of a long log) that a full-log plot would render too small to read. Trimmed output filenames get a `_trimSs-Es` suffix to avoid colliding with a full-log run; the report's Metadata section records the applied window and row count. See README.md's "Time-Window Trim: How To Pick `--start`/`--end`" for the recommended workflow and a worked desync example.
            * **Caveat — statistics computed within the window (IT #182):** Motor Desync Detection's `De Facto`/`Possible`/`Fallback` tiers (`src/plot_functions/motor_desync.rs`) score each motor against a baseline built only from the rows in the analyzed window — `detect_de_facto`'s own-motor high-command baseline (needs `MOTOR_DESYNC_MIN_BASELINE_SAMPLES` samples elsewhere in the window), `detect_possible`'s whole-window 90th-percentile eRPM reference, and `detect_fallback`'s whole-window 97th-percentile tracking-error threshold all shrink or shift as the window shrinks. A flag visible on the full-log run can disappear on a tightly trimmed run even though the `Motor_vs_eRPM` plot itself shows the event more clearly — trim wide enough to keep a representative baseline if the table flag matters, or read the plot directly for a tight trim. See IT #182 for the full per-feature audit (which other analyses are and aren't affected) and the proposed fix (compute the baseline from the full log, restrict only event search/reporting to the trimmed window).
            * **Caveat — Optimal P Estimation (`--estimate-optimal-p`):** trimming has no well-defined effect on this feature. Phase 1 profiling (`profile_aircraft_group`, torque-inertia ratio across the aircraft group) re-parses each input file from disk directly and always uses the full file, regardless of `--start`/`--end`; only the Phase 2 Td measurement (from step response) and D-term HF energy use the trimmed data. Don't rely on `--start`/`--end` to scope this feature — see IT #182.
