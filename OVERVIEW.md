        ...
        * **P:D Ratio Recommendations (`src/main.rs`):**
            ...
            * **Other plots generated (`src/plot_functions/`):** Which plots are generated is controlled by the `PlotConfig` struct. The default (no flag) enables the core set; `--extended` enables all plots except Bode. Plots are gated per-field in `PlotConfig`:
                ...
                * **Extended plots (`--extended` adds these to the core set):**
                    ...
                    * `plot_stick_distribution`: Per-axis time-domain trace of RC Command (stick position) deflection as % of peak, plus Setpoint-vs-Gyro rate utilization. See [Stick Position Distribution](#stick-position-distribution) below.
...
* **Purpose:** Reports how a flight's stick input was actually distributed and how closely the aircraft tracked it, as raw data — no tuning advisory (`src/plot_functions/plot_stick_distribution.rs`, IT #163).
* **Deflection zones:** Each RC Command sample is expressed as a % of that axis's own peak `|rc_command|` for the flight — a data-driven reference, independent of the configured-max-rate computation below. Center (<15%), Mid (15–75%), and High (>75%) zone time is reported as a % of total flight time, weighted by each sample's own held duration (the same sample-and-hold weighting `detect_rc_command_steps` uses for plateau timing) — not by sample count. Saturation (>95%) time is reported in seconds, as a subset of High.
* **Plot:** `*_Stick_Distribution_stacked.png` draws `rc_command` as a time-domain trace per axis — % of this flight's own peak deflection on the Y-axis, time on the X-axis, in the same visual style as `plot_rc_command_activity`'s Setpoint/RC Command overlay — with horizontal reference lines at the 15/75/95% zone boundaries.
...
* **Rate utilization:** P95 Setpoint vs. P95 Gyro Achieved per axis — the 95th percentile, not the raw maximum. A single crash/tumble sample can put raw max gyro rate an order of magnitude above every other sample in the flight (the same outlier problem `plot_setpoint_vs_gyro`'s Y-axis scaling guards against), which would make a max-based tracking figure meaningless. Tracking error is `|P95 Gyro - P95 Setpoint| / P95 Setpoint`, reported as N/A below `MOVEMENT_THRESHOLD_DEG_S` (20 deg/s, the same PTB/PlasmaTree-sourced "is there real movement" floor `calc_step_response.rs` already uses) — a near-idle axis has no real setpoint signal to divide by, and the ratio would turn ordinary gyro noise into a meaningless triple-digit percentage.
...
* **Report:** `## Stick Position & Rate Analysis` in the markdown report, `--extended` only. Omitted when the log has no RC Command samples with a positive peak.
