...
- [Code Overview and Step Response Calculation](#code-overview-and-step-response-calculation)
  ...
  - [Stick Position Distribution](#stick-position-distribution)
...
2.  **File Processing (`src/main.rs:process_file`):**
    * For each input CSV:
        ...
        * **Time-Window Trim (optional `--start`/`--end`):** When either flag is given, rows outside the requested window (seconds relative to the log's first row) are dropped from the in-memory dataset before any analysis or plot runs, so every module — step response, gyro/motor/D-term analysis — sees only the trimmed subset. Motor Desync Detection is the one exception: it also receives the full untrimmed dataset for its baseline statistics (see the note below). Useful for zooming into a short event (e.g. a brief motor desync near the end of a long log) that a full-log plot would render too small to read. Trimmed output filenames get a `_trimSs-Es` suffix to avoid colliding with a full-log run; the report's Metadata section records the applied window and row count. See README.md's "Time-Window Trim: How To Pick `--start`/`--end`" for the recommended workflow and a worked desync example.
            * **Motor Desync Detection baseline uses the full log (fixed, IT #182):** `detect_motor_desync` (`src/plot_functions/motor_desync.rs`) takes the full, untrimmed log plus the trim window separately. Every baseline/threshold — `detect_de_facto`'s own-motor high-command baseline (`MOTOR_DESYNC_MIN_BASELINE_SAMPLES` samples elsewhere in the *full* log), `detect_possible`'s 90th-percentile eRPM reference, `detect_fallback`'s 97th-percentile tracking-error threshold — is computed from the full log regardless of `--start`/`--end`, so it can't shrink or shift with a tight trim. Only which flagged events get reported is restricted to the trim window. Before this fix, the baseline was built from the trimmed rows only, so a window tight enough to make the `Motor_vs_eRPM` plot clearly readable could make the table flag disappear even though the event was still visible in the plot — see IT #182's original audit for the confirmed repro.
            * **Caveat — Optimal P Estimation (`--estimate-optimal-p`):** trimming has no well-defined effect on this feature. Phase 1 profiling (`profile_aircraft_group`, torque-inertia ratio across the aircraft group) re-parses each input file from disk directly and always uses the full file, regardless of `--start`/`--end`; only the Phase 2 Td measurement (from step response) and D-term HF energy use the trimmed data. Don't rely on `--start`/`--end` to scope this feature — no code change is planned for this (see IT #182).
        ...
        * **P:D Ratio Recommendations (`src/main.rs`):**
            ...
            * **Other plots generated (`src/plot_functions/`):** Which plots are generated is controlled by the `PlotConfig` struct. The default (no flag) enables the core set; `--extended` enables all plots except Bode. Plots are gated per-field in `PlotConfig`:
                * **Core plots (default — no flag required):**
                    ...
                    * `plot_rc_command_activity`: Setpoint vs. RC Command time-domain overlay for each axis, to visually correlate blocky/unfiltered stick input against the flight controller's response. Also runs RC Command step detection (see below) for the markdown report.
                ...
                * **Report-only statistics (`src/data_analysis/`, not gated by any plot flag — always computed, same as Metadata and PID Tuning):**
                    * `stick_distribution`: Per-axis stick position and rate-utilization statistics. See [Stick Position Distribution](#stick-position-distribution) below.
...
* **Purpose:** Reports how a flight's stick input was actually distributed and how closely the aircraft tracked it, as raw data — no tuning advisory (`src/data_analysis/stick_distribution.rs`). Report-only: not gated by `--core`/`--extended`/`--step`/`--bode`/`--desync`, same as Metadata and PID Tuning — always computed, regardless of plot selection. No plot is generated; `rc_command` is already visualized over time by `plot_rc_command_activity`, and a deflection histogram/CDF plot tried for this feature added confusion without surfacing information the report table doesn't already state directly.
* **Deflection zones:** Each RC Command sample is expressed as a % of true full-stick `|rc_command|` (500, the value both Betaflight and EmuFlight clamp `rcCommand[Roll/Pitch/Yaw]` to before the rate curve) — not this flight's own peak, so a flight that never reaches full stick cannot register Saturation just by exceeding a fraction of its own already-partial peak. Independent of the configured-max-rate computation below. Center (<15%), Mid (15–75%), and High (>75%) zone time is reported as a % of total flight time, weighted by each sample's own held duration (the same sample-and-hold weighting `detect_rc_command_steps` uses for plateau timing) — not by sample count. Saturation (>95%) time is reported in seconds, as a subset of High; Saturation Events counts separate excursions above that threshold (a state-transition count), not cumulative time — two short punches to full stick count as 2 events, not diluted or inflated by how long each one lasted. Peak Stick (the flight's own observed peak, as a % of true full stick) is reported alongside the zones for context.
* **Center reversal rate:** Counts RC Command sign changes while continuously inside the Center zone (a real transit out to Mid/High and back resets tracking, so it isn't miscounted as center jitter), reported in Hz — divided by time actually spent in the Center zone, not total flight time, so it isn't diluted by however much of the flight was spent outside Center.
* **Rate utilization:** P95 Setpoint vs. P95 Gyro Achieved per axis — the 95th percentile, not the raw maximum. A single crash/tumble sample can put raw max gyro rate an order of magnitude above every other sample in the flight (the same outlier problem `plot_setpoint_vs_gyro`'s Y-axis scaling guards against). No derived "tracking error" percentage is reported here: comparing the two P95 values as independent aggregates misses genuine per-instant divergence (and can look fine even when it isn't), while a true per-instant computation is dominated by normal filter/motor lag during fast stick transients — a second, cruder version of what the report's Step Response Analysis section already measures properly (P:D ratio, overshoot assessment, cross-validated against PIDtoolbox/PlasmaTree — see [Step-Response Comparison with Other Analysis Tools](#step-response-comparison-with-other-analysis-tools)) would be redundant at best and misleading at worst. The report's Stick Position & Rate Analysis section points back to Step Response Analysis for tracking quality instead of re-deriving it.
* **Configured max rate:** `src/data_analysis/rate_curve.rs` computes the setpoint firmware produces at full stick deflection, from the header's `rates_type`/`rc_rates`/`rc_expo`/`rates`/`rate_limits` fields. Implements all 5 Betaflight rate curve families (Betaflight, RaceFlight, KISS, Actual, Quick) — verified byte-identical to EmuFlight's own `fc_rc.c` for the 4 families EmuFlight also has (EmuFlight has no Quick; its `rates_type` enum tops out at Actual). `rates_type` defaults to `Betaflight` when the header key is absent — both firmwares' own compiled-in default, and the only type that predates the header key's introduction. `rate_limits` defaults to 1998 deg/s when absent (EmuFlight never logs this key; it applies a fixed 1998 deg/s ceiling in firmware instead of a configurable one). `None` when `rc_rates`/`rc_expo`/`rates` is missing or malformed. Rate Headroom is the unused portion of this value — 100% minus (P95 Setpoint as a % of Configured Max Rate) — still measured data, no "lower your rates" advisory.
* **Report:** `## Stick Position & Rate Analysis` in the markdown report, not gated by any plot selection flag. Omitted from the report only when the log has no RC Command samples with a positive peak for any axis.
...
- **`*_report.md`** — Structured markdown flight report written alongside PNGs on every run. Content is assembled from typed result structs returned by each analysis pass — no CSV re-reading. Sections: Metadata (firmware revision, craft name, PIDs, sample rate, gyroUnfilt source warning), Filter Configuration (per-axis LPF1/LPF2/IMUF/Pseudo-Kalman table, Dynamic Notch, RPM filter), PID Tuning P:D ratios, Step Response Analysis (Roll/Pitch: peak value, assessment, setpoint authority, P:D recommendations), Gyro Analysis (per-axis filtering delay with confidence, spectrum peaks), D-Term Analysis (per-axis filtering delay with N/A disambiguation, spectrum peaks), Motor Oscillation table (per-motor sliding-window spectrum check, catches a brief burst a whole-log average would dilute away), Motor Desync Detection (per-motor motor[N] vs eRPM[N] divergence table, self-relative to that same motor's own behavior elsewhere in the flight; De Facto/Possible confidence tiers require bidirectional DShot telemetry; a lower-confidence Fallback tier runs instead when it's absent), Stick Input Smoothness (RC Command step detection, with an rc_smoothing recommendation when an axis is classified Blocky), Stick Position & Rate Analysis (computed regardless of plot selection, not gated by `--core`/`--extended`/etc. — per-axis Center/Mid/High/Saturation zone-time percentages, Saturation Events, Center reversal rate, P95 Setpoint/P95 Gyro Achieved, Configured Max Rate and Rate Headroom from the header's rate-curve config — no tuning advisory; omitted from the report only when the log has no RC Command samples with a positive peak for any axis), and relative links to all generated PNGs. Optimal P Estimation and Bode Analysis sections are included when those features produce results.
