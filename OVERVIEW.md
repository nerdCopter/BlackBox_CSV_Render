...
2.  **File Processing (`src/main.rs:process_file`):**
    * For each input CSV:
        ...
        * **Time-Window Trim (optional `--start`/`--end`):** When either flag is given, rows outside the requested window (seconds relative to the log's first row) are dropped from the in-memory dataset before any analysis or plot runs, so every module — step response, gyro/motor/D-term analysis, desync detection — sees only the trimmed subset. Useful for zooming into a short event (e.g. a brief motor desync near the end of a long log) that a full-log plot would render too small to read. Trimmed output filenames get a `_trimSs-Es` suffix to avoid colliding with a full-log run; the report's Metadata section records the applied window and row count. Caveat: Motor Desync Detection (`src/plot_functions/motor_desync.rs`) flags each motor self-relative to that motor's own high-command history *within the analyzed window* — trimming to a short tail removes most of that baseline, so a flag visible on the full-log run can disappear from the Motor Desync Detection table on a tightly trimmed run even though the `Motor_vs_eRPM` plot itself shows the event more clearly. Trim wide enough to keep a representative baseline if the table flag matters, or read the plot directly.
