// src/plot_functions/plot_psd_heatmap.rs
...
use crate::plot_framework::{
    // Will need new types and functions from plot_framework
    draw_dual_heatmap_plot, // This will be the new top-level drawing function
    HeatmapPlotConfig,      // New struct for heatmap configuration
    AxisHeatmapSpectrum,    // New struct for axis-specific heatmap data
};
use crate::constants::{
    SPECTRUM_NOISE_FLOOR_HZ, // Reused for frequency filtering, though less critical for heatmap
    COLOR_GYRO_VS_UNFILT_UNFILT, COLOR_GYRO_VS_UNFILT_FILT, // Reused for general context, not direct plotting
    STFT_WINDOW_DURATION_S, STFT_OVERLAP_FACTOR, // NEW: For STFT windowing
...
pub fn plot_psd_heatmap(
    log_data: &[LogRowData],
    root_name: &str,
    sample_rate: Option<f64>,
) -> Result<(), Box<dyn Error>> {
    let output_file = format!("{}_Gyro_PSD_Heatmap_comparative.png", root_name);
    let plot_type_name = "Gyro PSD Heatmap";

    let sr_value = if let Some(sr) = sample_rate {
        sr
    } else {
        println!("\nINFO: Skipping Gyro PSD Heatmap Plot: Sample rate could not be determined.");
        return Ok(());
    };
    ...
    if window_size_samples == 0 {
        eprintln!("Error: Window size is zero, cannot perform STFT. Adjust STFT_WINDOW_DURATION_S.");
        return Ok(());
    }
    ...
    for axis_idx in 0..3 {
        ...
        for row in log_data {
            if let (Some(time), Some(unfilt_val), Some(filt_val)) = (row.time_sec, row.gyro_unfilt[axis_idx], row.gyro[axis_idx]) {
        ...
        if unfilt_time_series.len() < window_size_samples || filt_time_series.len() < window_size_samples {
            println!("  Not enough data for {} axis to perform STFT. Skipping PSD heatmap.", axis_name);
            continue;
        }
        ...
        let window_func = calc_step_response::tukeywin(window_size_samples, 1.0); // Hanning window (Tukey alpha=1.0)
        ...
        while current_start_sample + window_size_samples <= unfilt_time_series.len() {
            ...
            let window_time_center = time_stamps[current_start_sample] + STFT_WINDOW_DURATION_S / 2.0;
            
            let mut padded_unfilt = Array1::<f32>::zeros(fft_padded_len);
            padded_unfilt.slice_mut(s![0..window_size_samples]).assign(&(&Array1::from_vec(unfilt_window_slice.to_vec()) * &window_func));
            let mut padded_filt = Array1::<f32>::zeros(fft_padded_len);
            padded_filt.slice_mut(s![0..window_size_samples]).assign(&(&Array1::from_vec(filt_window_slice.to_vec()) * &window_func));
            ...
            for i in 0..num_unique_freqs {
                if i >= num_freq_bins_to_plot { break; } // Only process frequencies we intend to plot

                let mut amp_unfilt_psd = (unfilt_spec[i].re.powi(2) + unfilt_spec[i].im.powi(2)) as f64 * psd_scale;
                let mut amp_filt_psd = (filt_spec[i].re.powi(2) + filt_spec[i].im.powi(2)) as f64 * psd_scale;
                ...
                current_unfilt_psd_row.push(linear_to_db(amp_unfilt_psd));
                current_filt_psd_row.push(linear_to_db(amp_filt_psd));
            }
        ...
        if unfilt_psd_matrix.is_empty() {
            println!("  No valid STFT windows generated for {} axis. Skipping PSD heatmap.", axis_name);
            continue;
        }

        // Determine X-axis range for plotting (time)
        let x_range_plot = if time_x_bins.len() > 1 {
            time_x_bins.first().unwrap().clone()..time_x_bins.last().unwrap().clone()
        ...
        let y_range_plot = if frequencies_y_bins.len() > 1 {
            frequencies_y_bins.first().unwrap().clone()..frequencies_y_bins.last().unwrap().clone()
        ...
        let unfiltered_heatmap_config = HeatmapPlotConfig {
            ...
            heatmap_data: crate::plot_framework::HeatmapData {
