...
use crate::constants::{
    STFT_WINDOW_DURATION_S, STFT_OVERLAP_FACTOR,
    HEATMAP_MIN_PSD_DB, // Removed HEATMAP_MAX_PSD_DB from here
    TUKEY_ALPHA,
    THROTTLE_Y_BINS_COUNT, THROTTLE_Y_MIN_VALUE, THROTTLE_Y_MAX_VALUE,
};
...
pub fn plot_throttle_freq_heatmap(
    ...
    for axis_idx in 0..3 {
        ...
        for row in log_data {
            if let (Some(unfilt_val), Some(filt_val), Some(throttle_val)) = (row.gyro_unfilt[axis_idx], row.gyro[axis_idx], row.setpoint[3]) {
        ...
        let mut unfilt_psd_sums: Vec<Vec<f64>> = vec![vec![0.0; THROTTLE_Y_BINS_COUNT]; num_freq_bins_to_plot];
        let mut unfilt_psd_counts: Vec<Vec<usize>> = vec![vec![0; THROTTLE_Y_BINS_COUNT]; num_freq_bins_to_plot];
        let mut filt_psd_sums: Vec<Vec<f64>> = vec![vec![0.0; THROTTLE_Y_BINS_COUNT]; num_freq_bins_to_plot];
        let mut filt_psd_counts: Vec<Vec<usize>> = vec![vec![0; THROTTLE_Y_BINS_COUNT]; num_freq_bins_to_plot];
        ...
        while current_start_sample + window_size_samples <= unfilt_time_series.len() {
            ...
            let window_throttle_val = throttle_values[current_start_sample + window_size_samples / 2];
            
            // Map throttle value to a Y-axis bin
            let mut throttle_y_bin_idx = ((window_throttle_val - THROTTLE_Y_MIN_VALUE) / throttle_bin_size).floor() as usize;
            ...
            for i in 0..num_unique_freqs {
                ...
                unfilt_psd_sums[i][throttle_y_bin_idx] += linear_to_db_for_heatmap(amp_unfilt_linear_psd);
                unfilt_psd_counts[i][throttle_y_bin_idx] += 1;
                filt_psd_sums[i][throttle_y_bin_idx] += linear_to_db_for_heatmap(amp_filt_linear_psd);
        ...
        let mut final_unfilt_psd_matrix: Vec<Vec<f64>> = vec![vec![0.0; THROTTLE_Y_BINS_COUNT]; num_freq_bins_to_plot];
        let mut final_filt_psd_matrix: Vec<Vec<f64>> = vec![vec![0.0; THROTTLE_Y_BINS_COUNT]; num_freq_bins_to_plot];

        for f_idx in 0..num_freq_bins_to_plot {
            for t_idx in 0..THROTTLE_Y_BINS_COUNT {
                if unfilt_psd_counts[f_idx][t_idx] > 0 {
                    final_unfilt_psd_matrix[f_idx][t_idx] = unfilt_psd_sums[f_idx][t_idx] / unfilt_psd_counts[f_idx][t_idx] as f64;
                } else {
                    final_unfilt_psd_matrix[f_idx][t_idx] = HEATMAP_MIN_PSD_DB; // No data for this bin
                }
                if filt_psd_counts[f_idx][t_idx] > 0 {
                    final_filt_psd_matrix[f_idx][t_idx] = filt_psd_sums[f_idx][t_idx] / filt_psd_counts[f_idx][t_idx] as f64;
                } else {
