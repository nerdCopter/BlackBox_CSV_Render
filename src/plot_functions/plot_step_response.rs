            ...
            if show_legend {
                ...
                if let Some(resp) = final_low_response {
                    ...
                    series.push(PlotSeries {
                        ...
                        label: format!("< {} deg/s (Peak: {}, Latency: {})", setpoint_threshold, peak_str, latency_str),
                        color: color_low_sp,
                ...
                if let Some(resp) = final_high_response {
                    ...
                    series.push(PlotSeries {
                        data: response_time.iter().zip(resp.iter()).map(|(&t, &v)| (t, v)).collect(),
                        label: format!("\u{2265} {} deg/s (Peak: {}, Latency: {})", setpoint_threshold, peak_str, latency_str),
                        color: color_high_sp,
                        stroke_width: line_stroke_plot,
                    });
                }
                if let Some(resp) = final_combined_response {
                    ...
                    let latency_str = latency_opt.map_or_else(|| "N/A".to_string(), |l_s| format!("{:.0} ms", l_s * 1000.0));
                     series.push(PlotSeries {
                        data: response_time.iter().zip(resp.iter()).map(|(&t, &v)| (t, v)).collect(),
                        label: format!("Combined (Peak: {}, Latency: {})", peak_str, latency_str), // This is the average of all Y-corrected & QC'd responses
                ...
                if let Some(resp) = final_combined_response {
                    ...
                    series.push(PlotSeries {
                        data: response_time.iter().zip(resp.iter()).map(|(&t, &v)| (t, v)).collect(),
                        label: format!("step-response \u{2265} {} deg/s (Latency: {})", MOVEMENT_THRESHOLD_DEG_S, latency_str), // Label reflects initial movement filter
                        color: color_combined,
                        stroke_width: line_stroke_plot,
                    });
                }
