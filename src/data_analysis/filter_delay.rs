    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            DelayCalculationError::SignalMismatch => {
                write!(
                    f,
                    "Signal mismatch: filtered and unfiltered signals have different lengths"
                )
            }
        }
    }
...
pub fn calculate_filtering_delay(
    ...
    if !sample_rate.is_finite() || sample_rate <= 0.0 {
        return Err(DelayCalculationError::InvalidSampleRate { sample_rate });
    }
    ...
    if best_correlation > MIN_CORRELATION_THRESHOLD as f64 && best_delay > 0 {
        ...
        for delay in 1..((n / MAX_DELAY_FRACTION).min(MIN_SAMPLES_FOR_DELAY)) {
            ...
            if filtered_norm > 1e-10 && unfiltered_norm > 1e-10 {
                let normalized_corr =
                    correlation_sum / (filtered_norm.sqrt() * unfiltered_norm.sqrt());
...
pub fn calculate_average_filtering_delay_comparison(
    ...
    for axis in 0..AXIS_NAMES.len() {
        ...
        if !filtered_data.is_empty() && filtered_data.len() == unfiltered_data.len() {
            ...
            let axis_results = calculate_filtering_delay_comparison(
                &filtered_array,
                &unfiltered_array,
                sample_rate,
            );
    ...
    if !all_results.is_empty() {
        ...
        let enhanced_results: Vec<&DelayResult> = all_results
            .iter()
            .filter(|r| r.method == "Enhanced Cross-Correlation")
            .collect();
        if !enhanced_results.is_empty() {
            let avg_delay = enhanced_results.iter().map(|r| r.delay_ms).sum::<f32>()
                / enhanced_results.len() as f32;
            let avg_confidence = enhanced_results.iter().map(|r| r.confidence).sum::<f32>()
                / enhanced_results.len() as f32;
...
fn calculate_filtering_delay_comparison(
    ...
    if let Some(enhanced_result) =
        calculate_filtering_delay_enhanced_xcorr(filtered, unfiltered, sample_rate)
    {
        results.push(enhanced_result);
    } else {
        println!(
            "  Enhanced Cross-correlation method failed for {} samples",
            filtered.len()
        );
    }
