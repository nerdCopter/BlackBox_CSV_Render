...
/// Type alias for RPM filter curve data: (label, curve_points, center_hz)
#[allow(dead_code)] // Used in Phase 5 (visualization)
pub type RpmFilterCurveData = (String, Vec<(f64, f64)>, f64);
        ...
        if let Some(harmonics_str) = get_value("rpm_filter_harmonics") {
            if let Ok(harmonics) = harmonics_str.parse::<u32>() {
                if harmonics > 0 {
                    ...
                    let weights = if let Some(weights_str) = get_value("rpm_filter_weights") {
                        weights_str
                            .split(',')
                            .filter_map(|s| s.trim().parse::<f64>().ok())
                            .map(|w| w / 100.0) // Convert 0-100 to 0.0-1.0
                            .collect()
                    } else {
