...
fn get_spectrogram_color(value: f32, min_clip: f32, max_clip: f32) -> RGBColor {
    ...
    SPECTROGRAM_COLOR_SCALE.last().unwrap().1
}
...
where DB::ErrorType: 'static
{
    ...
    let x_range_spec = 0f32..100f32;
    let y_range_spec = 0f32..SPECTROGRAM_MAX_FREQ_HZ;
    ...
    let throttle_cell_width = if num_throttle_plot_bins > 0 { // Ensure not dividing by zero
    ...
    for j in 0..num_throttle_plot_bins {
        ...
        for i in 0..num_freq_bins_total {
            let freq_center = freq_bins[i];
            if freq_center > SPECTROGRAM_MAX_FREQ_HZ + 1e-3 {
                continue;
            }

            let freq_cell_height = if num_freq_bins_total > 0 { // Guard division by zero
                 if i + 1 < num_freq_bins_total {
                    freq_bins[i+1] - freq_bins[i]
                } else if i > 0 {
                    freq_bins[i] - freq_bins[i-1]
                } else {
                    SPECTROGRAM_MAX_FREQ_HZ / num_freq_bins_total as f32
                }
            } else {
                SPECTROGRAM_MAX_FREQ_HZ // Should not happen if guarded above
            };
    ...
    let text_style_spec = ("sans-serif", 12).into_font().color(SPECTROGRAM_TEXT_COLOR); // Renamed
...
where DB::ErrorType: 'static
{
    ...
    let mut chart = ChartBuilder::on(area)
        ...
        .margin(5)
        .build_cartesian_2d(0f32..SPECTROGRAM_POWER_CLIP_MAX, 0f32..1f32)?;
...
pub fn plot_throttle_spectrograms(
    ...
    let sr = match sample_rate {
        Some(s_rate) => s_rate, // Renamed s to s_rate
        None => {
    ...
    let _plot_area_height = PLOT_HEIGHT - main_title_area_height - colorbar_area_height - 20;
    ...
    draw_spectrogram_colorbar(&colorbar_area.margin(0, 100, 0, 100), "Scale (Power)")?;
